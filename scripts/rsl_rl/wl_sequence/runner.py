...
class WlSequenceRunner:
    ...
    def _leg_debug_stats(self) -> dict[str, torch.Tensor]:
        ...
        return {
            ...
            "theta0_abs_mean": vmc_state["theta0"].abs().mean(),
            "theta0_abs_max": vmc_state["theta0"].abs().max(),
            "theta0_ready_frac_0p35": vmc_state["theta0"].abs().lt(0.35).all(dim=1).float().mean(),
            "theta0_lr_error_abs_mean": (vmc_state["theta0"][:, 0] - vmc_state["theta0"][:, 1]).abs().mean(),
            "L0_mean": vmc_state["L0"].mean(dim=0),
            "L0_abs_mean": vmc_state["L0"].abs().mean(),
    ...
    def _write_diagnostics_jsonl(
        ...
        row = {
            ...
            "posture": {
                ...
                "theta0_abs_mean": self._as_float(leg_stats["theta0_abs_mean"]),
                "theta0_abs_max": self._as_float(leg_stats["theta0_abs_max"]),
                "theta0_ready_frac_0p35": self._as_float(leg_stats["theta0_ready_frac_0p35"]),
                "theta0_mean": self._as_list(leg_stats["theta0_mean"]),
                "theta0_lr_error_abs_mean": self._as_float(leg_stats["theta0_lr_error_abs_mean"]),
                "L0_mean": self._as_list(leg_stats["L0_mean"]),
                "L0_abs_mean": self._as_float(leg_stats["L0_abs_mean"]),
    ...
    def _log_diagnostics_to_tensorboard(
        ...
        scalars = {
            ...
            "Diagnostics/theta0_abs_mean": leg_stats["theta0_abs_mean"],
            "Diagnostics/theta0_abs_max": leg_stats["theta0_abs_max"],
            "Diagnostics/theta0_ready_frac_0p35": leg_stats["theta0_ready_frac_0p35"],
            "Diagnostics/theta0_lr_error_abs_mean": leg_stats["theta0_lr_error_abs_mean"],
            "Diagnostics/L0_abs_mean": leg_stats["L0_abs_mean"],
